...
custom_bars <- function(data, x, y, percent = NULL, margin = 15, angle_rotation = 45, v_just = 0.5, x_title_size = 8, legend_size = 7) {
  x <- enquo(x)
  y <- enquo(y)
  percent_quo <- enquo(percent)
  if (length(data %>% select(!!percent_quo)) != 0)add_percent <- TRUE else add_percent <- FALSE

  
  n_bars <- data %>% select(!!x) %>% unlist() %>% levels() %>%  length()
  ...
  ggplot(aes(x=!!x, y = !!y)) +
      ...
      theme(panel.border = element_rect(linetype = "blank", size= 0.9, fill = NA),
            ...
            if (add_percent != FALSE)
              geom_text(aes(label=paste0(!!y ," (",scales::percent(!!percent_quo),")")), vjust=-0.5)
            else
              geom_text(aes(label=!!y), vjust=-0.5, size = label_size) 
}
```
...
parse_categories <- function(data) {
    ...
    function(field_name, field_label, options) {
     list(
       field_name = field_name,
        field_label = field_label,#mapply(function(x, y) y, field_name, field_label, SIMPLIFY = FALSE),
  ...
  #   function(row) {
  #     print(row)
  #     options_matrix <- str_split(row["select_choices_or_calculations"], "[|]") #%>%
  #       #(function (x) gsub("^\\s+|\\s+^", "", x)) #%>%
  #       # lapply(str_split, pattern = ",", n=2, simplify = TRUE) #%>%
  #       #lapply(function (x) gsub("^\\s+|\\s+^", "", x))
  #       # lapply(strsplit(unname(row["select_choices_or_calculations"]), "[|]"), 
  #       #               # Remove leading/trailing whitespace from "  option_code, option_label "
  #       #               function(x) {gsub("^\\s+|\\s+^", "", x)}) %>%
  #       #               # Split "option_code, option_label" over ","
  #       #             lapply(
  #       #               # Remove whitespace from "option_code ", " option_label"
  #       #               function(x) {gsub("^\\s+|\\s+^", "", str_split(x, pattern = ",", n=2, simplify = TRUE))})
  #     print(options_matrix)
  #     # list(
  #     #   field_name = row["field_name"],
  #     #   field_label = row["field_label"],
  #     #   options_code = options_matrix[,1],
  #     #   options_label = mapply(function(x, y) y,options_matrix[,1], options_matrix[,2])
  #     # )
  #   }
  # )
}

# split_codes_and_labels <- function(options) {
#                     # Split all options over "|" character
#   options_matrix <- lapply(strsplit(options, "[|]"), 
#                       # Remove leading/trailing whitespace from "  option_code, option_label " 
#                       function(x) {gsub("^\\s+|\\s+^", "", x)}) %>%
#                       # Split "option_code, option_label" over ","
#                     lapply(
...
#                       function(x) {gsub("^\\s+|\\s+^", "", str_split(x, pattern = ",", n=2, simplify = TRUE))})
#   
#   # Return columns containing the corresponding list of codes and options for each field_name
#   data.table(
#     options_code = lapply(options_matrix, function(x) x[,1]), 
#     options_label = lapply(options_matrix, function(x) x[,2])
#   )
# }
...
for (checkbox_field in checkbox_fields) {
  # Set the names of the labels to their corresponding report field name
  #names(checkbox_field$options_label) <- paste0("___", checkbox_field$options_code)
  
  # Get the number of options for this field
  n_bars <- length(checkbox_field$options_label)
  ...
  if (any(grepl(paste0(checkbox_field$field_name, "___[0-9]+\\b"), names(report_data)))) {
    checkbox_plot <- report_data %>%
      ...
      transmute("{checkbox_field$field_label}" := factor(checkbox_field$options_label[str_extract(name, "[0-9]+\\b")], levels = checkbox_field$options_label), count, percent) %>%
      custom_bars(x = !!sym(checkbox_field$field_label), y = count)
