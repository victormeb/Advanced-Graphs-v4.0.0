...
custom_scatter <- function(data) {
  field_labels <- unlist(names_to_labels[names(data)])
  data %>%
    na.omit() %>%
    plot(
...
numerical_data_fields <- report_data %>%
  ...
  select(where(is.numeric) 
         # Where the column isn't completely empty
         & where(function(x) !(all(is.na(x)))) 
         # And neither the the field_type nor the text_validation are NULL
         & where(function(x) 
           !any(
             is.na(data_dictionary[data_dictionary$field_name %in% names(x),  "text_validation_or_show_slider_number"])
               & data_dictionary[data_dictionary$field_name %in% names(x), "field_type"] == "text")
           )
         & -matches(paste0("*", ignored_names, "*", collapse = "|"))
         ) %>% names()
  # Only get field names
# For each field to be considered x
for (x_field in numerical_data_fields) {
  ...
  for (y_field in numerical_data_fields) {
    ...
    if (y_field != x_field)
      custom_scatter(report_data[, c(x_field, y_field)])
  } 
}
...
date_data_fields <- data_dictionary %>%
  ...
  filter(substr(text_validation_type_or_show_slider_number, 1, 4) == "date"
         # and the column isn't empty
         & where(function(x) !(all(is.na(x))))) %>%
...
# For each date_field as x
for (date_field in date_data_fields) {
  # And numeric_field as y
  for (numeric_field in numerical_data_fields) {
      custom_time_scatter(report_data[,c(date_field, numeric_field)])
  }
}
