...
# Fix duplicate label names
  data_dictionary[data_dictionary$field_name %in% names(report_data), "field_label"] <- vctrs::vec_as_names(gsub(pattern, "\\1", data_dictionary[data_dictionary$field_name %in% names(report_data), "field_label"]), repair = "unique", quiet = TRUE)
...
if (length(checkbox_fields) > 0 || length(radio_categories) > 0)
