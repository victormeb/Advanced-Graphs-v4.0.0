...
checkbox_fields <- data_dictionary %>% 
  ...
  transmute(
    field_name,
    field_label,
    options = lapply(strsplit(select_choices_or_calculations, "[|]"), function(x) {gsub("^\\s+|\\s+^", "", x)}),
    options_code = lapply(options, function(x) {gsub("^\\s+|\\s+^", "", substr(x, 1, regexpr(",", x)[1]-1))}),
    options_label = lapply(options, function(x) {gsub("^\\s+|\\s+^", "", substr(x, regexpr(",", x)[1]+1, nchar(x)))})
...
for (checkbox_field in checkbox_fields) {
  names(checkbox_field$options_label) <- checkbox_field$options_code
  checkbox_field = checkbox_fields[[1]]
  report_data %>%
    select(id, city_bc, health_authority) %>%
    #select(matches(paste0(checkbox_field$field_name, "__[0-9]+\\b")))) %>%
    pivot_longer(cols = everything()) %>%
    transmute(name = checkbox_field$options_label[str_extract(name, "[0-9]+")], value = value) %>%
    group_by(name) %>%
