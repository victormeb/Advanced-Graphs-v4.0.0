...
for (checkbox_field in checkbox_fields) {
  ...
  if (any(grepl(paste0(checkbox_field$field_name, "___[0-9]+\\b"), names(report_data)))) {
    (report_data %>%
      ...
      custom_bars(x = !!sym(checkbox_field$field_label), y = count, label2 = percent, percent = TRUE)) %>% print_single(paste0("Count of ", checkbox_field$field_label))
  }
}
...
for (radio_category in radio_categories) {
  ...
  (report_data %>%
    ...
    # Print it
    print_single(title = paste0("Count of ", radio_category$field_label))
}
