...
other_fields <- data_dictionary %>%
  ...
  (function(data) {
    mapply(
      function(field_name, other_field) {
        list(
          # The other field is the name of the current field
          other_fields = field_name,
          # Remove numbers from radio fields and remove square brackets
          field_names = lapply(other_field, gsub, pattern = "\\[|\\]|\\([0-9]+\\)", replacement = "")
        )
      },
      # Get the field name
      data$field_name,
      # Extract field names from in between [] brackets
      regmatches(data$branching_logic, gregexpr("\\[.*?\\]",data$branching_logic)),
      SIMPLIFY = FALSE
     ) %>%
      ## Reverse the order of the list so the names of the list refer to the
      # columns that have an associated other column
      enframe() %>%
      unnest(cols = value) %>%
      group_by(value) %>%
      summarize(name = list(name)) %>%
      transmute(field_name = value, other_fields = name) %>%
      deframe()
    })
      ...
      transmute(
       ...
       (function(data) {
         ...
         if (any(checkbox_field$field_name %in% names(other_fields)))
           for (other_field in other_fields[[checkbox_field$field_name]]$others)
             kable(report_data[[other_field$field]]) %>% print()
       })
    ...
    (function(data) {
         side_by_side(
          ...
          if (any(radio_category$field_name %in% names(other_fields)))
           for (other_field in other_fields[[radio_category$field_name]]$others)
             kable(report_data[[other_field$field_label]]) %>% print()
       })
}
...
#   for (category in radio_categories)
#     if (all(duplicated(report_data[, c(coordinate_field$longitude_name, coordinate_field$latitude_name, category$field_name)])|duplicated(report_data[, c(coordinate_field$longitude_name, coordinate_field$latitude_name, category$field_name)],fromLast = TRUE)))
