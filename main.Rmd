...
# Create a list of checkbox field parameters to pass to the 
# checkbox for loop
checkbox_fields <- data_dictionary %>%
  ...
  transmute(
    field_name,
    field_label,
    split_codes_and_labels(select_choices_or_calculations)
...
for (checkbox_field in checkbox_fields) {
  #print(checkbox_field$field_name)
  ...
  if (any(grepl(paste0(checkbox_field$field_name, "___[0-9]+\\b"), names(report_data)))) {
    checkbox_plot <- report_data %>%
      ...
      transmute("{checkbox_field$field_label}" := factor(checkbox_field$options_label[str_extract(name, "___[0-9]+\\b")], levels = checkbox_field$options_label), count, percent) %>%
      ggplot(aes(x=!!sym(checkbox_field$field_label), y = count)) +
      ...
      theme(panel.border = element_rect(linetype = "blank", size= 0.9, fill = NA),
            ...
            scale_y_continuous(expand = expansion(mult = 0.2)) +
                ...
                if(n_bars > 12 )
                  ...
                  geom_text(aes(label=paste(count ," (",scales::percent(percent),")")), vjust=-0.5, size = 3)
                else
                  geom_text(aes(label=paste(count," (",scales::percent(percent),")")), vjust=-0.5)
