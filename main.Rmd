...
# Likert colour lower colours
bottom_colors <- c('red3', 'orangered', 'tomato')
top_colors <- c('forestgreen', 'palegreen3', 'green', 'lightgoldenrod')
...
for (group_index in 1:nrow(likert_groups)) {
  ...
  # category_colours <- if (option_count > 1) c(likert_colors[1:floor((option_count+1)/2)], rev(top_colors[length():floor((option_count+1)/2)])) else "white"
...
#   names()
# 
# numerical_data_fields <- numerical_data_fields[!(numerical_data_fields %in% ignored_names)]
# 
...
# 
# empty <- apply(numeric_pairs, 1, function(name) {report_data %>% select(all_of(name), -one_of(ignored_names)) %>% na.omit() %>% plot})
...
#     plot(as.Date(report_data[,date_field]), report_data[,numeric_field])
#   }
# }
