...
if (nrow(live_filters) > 0 ) {
  cat("\n######", paste0(live_filters$field_title, " = ", live_filters$option_name, collapse = "\n######"))
} else {
  cat("\n###### None")
}
```
...
for (category in radio_categories) {
  ...
  for (numeric_field in numeric_fields) {
    ...
    report_data %>%
      ...
      summarise(sum = sum(numeric, na.rm = TRUE), mean = if (n() == 0) 0 else mean(numeric, na.rm = TRUE)) %>%
      (function (data){
...
```{r network_graphs, out.width = "100%", fig.align="center", eval = T}
