...
knitr::opts_chunk$set(echo = FALSE, eval = FALSE, results = "asis")
...
likert_groups <- data_dictionary %>%
  ...
  mutate(form_label = instruments[instruments[["instrument_name"]] == form_name, "instrument_label"]) %>%
  # Split into several dataframes by form name
  split(.$form_label, drop = TRUE) %>%
...
```{r likert_plots, eval = T}
for (i in seq_along(likert_groups)) {
  cat("<h3>Likert plots for Instrument(s) ", names(likert_groups)[i], "</h3>", sep = "")
  
  for (likert_group in likert_groups[[i]]) {
    report_data %>%
      data.frame() %>%
