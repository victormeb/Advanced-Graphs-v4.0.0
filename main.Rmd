...
custom_scatter <- function(data, field_labels = NULL, date_fields = c()) {
  ...
  data %>%
    na.omit() %>%
    mutate(across(date_fields, as.Date, tryFormats = c("%m/%d/%Y", "%Y-%m-%d", "%Y/%m/%d"))) %>%
    (function(data) {
      if (nrow(data) != 0) {
        plot(
          data,
        ...
        if (length(date_fields) == 0) {
          ...
          axis.Date(side = which(names(data) == date_fields), data[[date_fields]], format = "%m/%d/%Y", labels = TRUE, las = 2)
          axis(side = which(names(data) != date_fields), las = 1)
        } else {
          axis.Date(side = 1, data[[date_fields[1]]], format = "%m/%d/%Y", labels = TRUE, las = 2)
          axis.Date(side = 1, data[[date_fields[2]]], format = "%m/%d/%Y", labels = TRUE, las = 2)
        }
      }
    })
}
...
for (date_field in date_data_fields) {
  ...
  for (numeric_field in numerical_data_fields) {
      custom_scatter(report_data[,c(date_field, numeric_field)], names_to_labels[c(date_field, numeric_field)], date_fields = date_field)
    custom_scatter(report_data[,c(numeric_field, date_field)], names_to_labels[c(numeric_field, date_field)], date_fields = date_field)
  }
}
