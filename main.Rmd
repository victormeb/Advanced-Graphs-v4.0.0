...
import_data <- function(parameters, live_filters) {
  ...
  return(
    ...
    read.csv(text = data_filter, header = TRUE, sep = ",", stringsAsFactors = FALSE)
  )
}
...
post_project_info <- function (parameters) {
  ...
  postForm(
    ...
    read.csv(text = project_info, header = TRUE, sep = ",", stringsAsFactors = FALSE)
}
...
post_data_dictionary <- function (parameters) {
  ...
  postForm(
   ...
   .opts = RCurl::curlOptions(ssl.verifypeer = FALSE, ssl.verifyhost = FALSE, verbose=FALSE)
    ...
    read.csv(text = dd, header = TRUE, sep = ",", stringsAsFactors = FALSE)
}
...
parse_categories <- function(data_dictionary, categorical = dd_df$field_type %in% c("radio", "dropdown", "yesno", "truefalse", "checkbox")) {
  # Create a list of categories and their options
  categories <- 
    # split the variables by | "The pipe character"
    strsplit(data_dictionary$select_choices_or_calculations[categorical], "[|]") %>% 
