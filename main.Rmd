...
knitr::opts_chunk$set(echo = FALSE, eval = F, results = "asis")
...
for (category in radio_categories) {
  ...
  for (numeric_field in numeric_fields) {
    ...
    report_data %>%
      ...
      summarise(sum = sum(numeric, na.rm = TRUE), mean = if (n() == 0) 0 else round(mean(numeric, na.rm = TRUE), 2)) %>%
      (function (data){
...
```{r stacked_bars, results='asis', message=FALSE, eval = T}
...
maps <- lapply(coordinate_fields,
              function(coordinate_field) {
                ...
                report_data %>%
                  ...
                  # Remove na entries
                  na.omit() %>%
                  # group by pairs of longitudes and latidudes
                  group_by(across(everything())) %>%
                  # Add a location id to each latidude/longitude combination
                  # and add a count for the number of entries per location
                  summarise(location = cur_group_id(), count = n()) %>%
                  # Ungroup the output
                  ungroup() %>%
                  # Create a custom map
                  custom_map(lng = coordinate_field$longitude_label, lat = coordinate_field$latitude_label, location = "location", count = "count")
              }
)
