...
for (x_field in numerical_data_fields) {
  ...
  for (y_field in numeric_pairs[,2]) {
...
for (date_field in date_data_fields) {
  if (!all(is.na(report_data[,date_field]))) {
    for (numeric_field in numerical_data_fields) {
      if (!all(is.na(report_data[,date_field]))) {
        plot(as.Date(report_data[,date_field], tryFormats = c("%m/%d/%Y", "%Y-%m-%d", "%Y/%m/%d")), report_data[,numeric_field])
      }
    }
  }
}
