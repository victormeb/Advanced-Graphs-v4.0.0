...
import_data <- function(parameters, record_id, live_filters) {
  ...
  return(
    report_data %>%
      inner_join(live_filtered_records, by = record_id)
  )
}
...
# TODO: split likert into two sections
# Get the levels for each categorical field
likert_candidate_levels <- sapply(likert_categories, function(x) {categories[[x]][["label"]]})
print(likert_candidate_levels)
# Send the lists to 

# TODO: Other check like making sure common categories share an instrument

# For each group of variables that shares options create a likert_group
 likert_groups <- data.table(group_names = sapply(likert_candidate_levels, toString))[, list(list(.I)), by = group_names]# %>%
#   transmute(options = likert_candidate_levels, indices = V1, categories = lapply(V1, function(x) {likert_categories[x]}))
print(likert_groups)
# Create a graph for each likert_group
# for (likert_group in likert_groups) {
#   likert_dataframe <- lapply(report_data[likert_group$categories], factor, )
# }
