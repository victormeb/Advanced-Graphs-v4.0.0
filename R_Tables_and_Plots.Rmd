...
library(igraph)
library(sjmisc) # to test in text network if dataframe is empty

#############################

# Test to customize errors warnings & messages (not used)
...
ggplot_mine <- function(totals, x_factor, y_values, x_bars_lables, full_line, x_size_bars, x_size, y_size, x_label, y_label, x_face, y_face, palette_bar_amount) {
   ## You need to expand palette size
   colourCount = length(x_factor)
   getPalette = colorRampPalette(brewer.pal(8, palette_bar_amount))
   # finalPalette = getPalette(colourCount)
   result <- ggplot(data=totals, aes(x = x_factor, y = y_values, fill =  x_bars_lables) ) +  
   geom_bar(color="black", stat = "identity") + 
   # Now you can draw your all different factors
   scale_fill_manual(values = colorRampPalette(brewer.pal(8, palette_bar_amount))(colourCount)) +
   # Next line works only for few bars (8 or 12 max, depending on the pallete)
   # scale_fill_brewer(palette= palette_bar_amount) + 
   # In 'format' below, big.mark inserts a comma for thousands and scientific = FALSE avoids scientific format while
   # trim=FALSE removes possible spaces added in the conversion
   geom_text(label= format(y_values,big.mark=",",scientific = FALSE, trim=TRUE), vjust=-0.4, size = x_size_bars ) + 
   ...
   theme(panel.border = element_rect(linetype = "blank", size= 0.9, fill = NA),
        ...
        axis.text.x = element_text(size=x_size, face = x_face,
...
for(var_dd in 1:dd_len) {
  ...
  if( !is.na(choices_calc_dd[var_dd]) && 
   (field_type_dd[var_dd] == "radio" || field_type_dd[var_dd] =="dropdown" || field_type_dd[var_dd] == "yesno" || field_type_dd[var_dd] == "truefalse" || field_type_dd[var_dd] == "checkbox") ) {
...
no_results <- ""
# Message for maps, not used really
no_results_map <- ""
#print(c("1 no_results", no_results))
...
data_report_size <- 0
col_names_report <- ''
data_report_matrix <- NULL
# print(c("2 no_results", no_results))
# When data_report is equal to "\n" it doesn't have any data
 if(data_report != "\n" )  {
  report_cols_len <- 0
  col_names_report <- ''
 ...
 no_results <- "No data in report"
 }

# print(c("3 no_results", no_results))
...
if(no_results == "") {
  ...
  if(field_name_dd[1]%in% names(data_report_matrix) || filter_condition == "") {
                  ...
                  if(is.na(grepl(tolower(options_label[j,opt_found]), 
                      ...
                      grepl(key_likert_words[key_word], options_label[j,opt_found], ignore.case = TRUE) ) {
                          ...
                          if(   isTRUE(all.equal( options_label[j,1:10], 
                            ...
                            # if(is.null(data_report_matrix[,i]))
    ...
    print(c("likert_pct", likert_pct))
        ...
        graph_likert <- psychTools::dfOrder(graph_likert, ncol(graph_likert):1)
            ...
            if(( i > 0 )) {
              ...
              if(col_names_report[i] == field_name_dd[j]) {
                   ...
                   !grepl('latitud',  field_name_dd[j]) && !grepl('longitud',  field_name_dd[j]) )
               {
                ...
                if(first_field_report > 0 && second_field_report > 0)
                {
                  # TODO: XYZ The next condition is failing when there is only one numeric field 
                  # (as an example a numeric field was added in a MAiD report)
                      ...
                      !is.na( data_report_matrix[,second_field_report] ) )
                  {
                    ...
                    ### XYZ TODO: New error, 'origin' must be supplied in as.Date(). Added but still not working
                    ### Use pid 330 and project 1467
                       ...
                       substr(text_val_dd[first_field_dd],1,4) == "date")
                    {
                      data_report_first_field <-
                       as.Date(data_report_matrix[,first_field_report], origin ='1900-01-01', "%Y-%m-%d")
                      print(c("matrix", data_report_matrix[,first_field_report]))
                      print(c("1950-10-10", as.Date("1950-10-10"  , origin ='1900-01-01', "%Y-%m-%d")))
                      print(c("data_report_matrix[1,first_field_report]", as.Date(data_report_matrix[1,first_field_report]  , origin ='1900-01-01', "%Y-%m-%d")))
                       ...
                       substr(text_val_dd[second_field_dd],1,4) == "date")
                    {
                      data_report_second_field <-
                       as.Date(data_report_matrix[,second_field_report], origin ='1990-01-01', "%Y-%m-%d")
                      print(c("matrix", data_report_matrix[,second_field_report]))
                      print(c("1950-11-12", as.Date("1950-11-12"  , origin ='1990-01-01', "%Y-%m-%d")))
                      print(c("data_report_matrix[10,second_field_report]", data_report_matrix[10,second_field_report], as.Date(data_report_matrix[10,second_field_report]  , origin ='1990-01-01', "%Y-%m-%d")))
                      # Value for type of line, a segment
                      type_line <- "l"
                      # flag for date field
                      # XYZ: TODO Commented flag because causes error below in Date axis.
                      #is_date_second <- TRUE
                    }
                    else {
                      data_report_second_field <-
                    ...
                    points_to_plot <- psychTools::dfOrder(points_to_plot_mat)
                           ...
                           incomparables = FALSE )) > 2 )
                    {
                      ...
                      if( is_date_first == TRUE && is_date_second == TRUE) {
                        axis.Date(side = 2, data_report_first_field, origin = '1990-01-01',
                                format = "%y %m %d", labels = TRUE, las = 1)
                        axis.Date(side = 1, data_report_second_field, origin = '1990-01-01',
                                format = "%y %m %d", labels = TRUE, las = 2)
                      }
                      else if( is_date_second == TRUE ) {
                        axis.Date(side = 1, data_report_second_field, origin = '1990-01-01',
                                format = "%yd %m %d", labels = TRUE, las = 2)
                        axis(side = 2, labels = TRUE, las = 1)
                      }
                      else if( is_date_first == TRUE ) {
                        axis.Date(side = 1, data_report_first_field, origin = '1990-01-01',
                                format = "%y %m %d", labels = TRUE, las = 2)
                        axis(side = 2, labels = TRUE, las = 1)
                      }
                      else {
...
```{r bar graphs , echo=FALSE, out.width = "100%", fig.align="center"}
...
if(no_results == "") {
        ...
        for(i in 1:report_cols_len) {
          ...
          if(field_type_dd[j] == 'checkbox') {
            ...
            if(col_names_report[i] != field_name_dd[j]) {
              ...
              for(r in 1:report_cols_len) {
                ...
                if(found_condition_index > 0 && col_names_report[r] == field_name_dd[found_condition_index]) {
                 ...
                 for(other_loop in 1:number_others[count_table]) {
                  ...
                  # If there are comments linked to "Other" option, it prepares the list to print
                   if(number_others[count_table] > 0) {
                    ...
                    # print(other_summary) # doesn't work
                   }
              ...
              # Will produce graph only if there are data, nrow(data_w_legend) is the number of bars to plot
              if(nrow(data_w_legend) > 0) {
          ...
          if(field_type_dd[j] == 'radio' || field_type_dd[j] == 'dropdown'
              || field_type_dd[j] == 'yesno' || field_type_dd[j] == 'truefalse') {
          # Checks if field name in column and dd are the same
            if(col_names_report[i] == field_name_dd[j]) {
              # Assigns field indexes
              if(field_report == 0) {
              ...
              if(field_type_dd[field_dd] == "yesno" ) levels(field_report_factors) <- c("No","Yes")
              if(field_type_dd[j] == "truefalse" ) levels(field_report_factors) <- c("False","True")
                cross.table.df <- data.frame(field_report_factors)
                max_label_size <- 50
                # If there is a positive number of options
                if(!isTRUE(options_number[field_dd] == "") ) {
               ...
               data_w_legend <- data.frame(field_report_factors)
                # print(c("field_name_dd[", field_name_dd[field_dd]))
                # print(c("field_report_factors", field_report_factors))
                ...
                if(nrow(data_w_legend) > 0) {
                  ...
                  legend_size = 7
                  #########################
                  # Number of bars (distinc values, including NA)
                  n_bars = length(unique(field_report_factors) )
                  # # Plus one if there are any NAs
                  # print( c("unique(field_report_factors", unique(field_report_factors)))
                  # print( c("data w legend", data_w_legend))
                  # print( c("is.na(data w legend)", is.na(data_w_legend)))
                  # print( c("any(is.na(data w legend))", any(is.na(data_w_legend))) )
                  # print( c("n_bars", n_bars))
                  # If too many bars, adjusts x labels parameters
                  if(n_bars > 34 ) {
                    angle_rotation = 90
                    vertical_just = 0
                  ...
                  if(n_bars > 12 )
                    ...
                    geom_text(aes(label=paste(..count..," (",scales::percent(..count../sum(..count..)),")")),
                    stat='count', vjust=-0.5)
           # Actually prints the resulting graph
           print(plot_levels)
                  field_report <- 0
                } # col_names_report[i]
             }
  ...
  # Workaround to print graphs. Prints as many graphs as needed.
  # Second 'if' is to plot  if have data
  # This part not needed anymore
  # if(report_len > 0 & count_table>=1 ) if( is.null(plot_levels_1_1) == FALSE ) plot_levels_1_1
  # if(report_len > 0 & count_table>=2 ) if( is.null(plot_levels_1_2) == FALSE ) plot_levels_1_2
  # if(report_len > 0 & count_table>=3 ) if( is.null(plot_levels_1_3) == FALSE ) plot_levels_1_3
  # if(report_len > 0 & count_table>=4 ) if( is.null(plot_levels_1_4) == FALSE ) plot_levels_1_4
  # if(report_len > 0 & count_table>=5 ) if( is.null(plot_levels_1_5) == FALSE ) plot_levels_1_5
  # if(report_len > 0 & count_table>=6 ) if( is.null(plot_levels_1_6) == FALSE ) plot_levels_1_6
  # if(report_len > 0 & count_table>=7 ) if( is.null(plot_levels_1_7) == FALSE ) plot_levels_1_7
  # if(report_len > 0 & count_table>=8 ) if( is.null(plot_levels_1_8) == FALSE ) plot_levels_1_8
  # if(report_len > 0 & count_table>=9 ) if( is.null(plot_levels_1_9) == FALSE ) plot_levels_1_9
  # if(report_len > 0 & count_table>=10 ) if( is.null(plot_levels_1_10) == FALSE ) plot_levels_1_10
  # if(report_len > 0 & count_table>=11 ) if( is.null(plot_levels_1_11) == FALSE ) plot_levels_1_11
  # if(report_len > 0 & count_table>=12 ) if( is.null(plot_levels_1_12) == FALSE ) plot_levels_1_12
  # if(report_len > 0 & count_table>=13 ) if( is.null(plot_levels_1_13) == FALSE ) plot_levels_1_13
  # if(report_len > 0 & count_table>=14 ) if( is.null(plot_levels_1_14) == FALSE ) plot_levels_1_14
  # if(report_len > 0 & count_table>=15 ) if( is.null(plot_levels_1_15) == FALSE ) plot_levels_1_15
  # if(report_len > 0 & count_table>=16 ) if( is.null(plot_levels_1_16) == FALSE ) plot_levels_1_16
  # if(report_len > 0 & count_table>=17 ) if( is.null(plot_levels_1_17) == FALSE ) plot_levels_1_17
  # if(report_len > 0 & count_table>=18 ) if( is.null(plot_levels_1_18) == FALSE ) plot_levels_1_18
  # if(report_len > 0 & count_table>=19 ) if( is.null(plot_levels_1_19) == FALSE ) plot_levels_1_19
  # if(report_len > 0 & count_table>=20 ) if( is.null(plot_levels_1_20) == FALSE ) plot_levels_1_20

  # Workaround to print "other" options Prints as many tables as needed.
...
```{r "Bars for Numeric Values", echo=FALSE, fig.height=9, fig.width=10, fig.align="center"}

######################################
# Bars for Numeric Values
######################################
...
#if(1==0) {
if(no_results == '') {
  # If report has record id or it's not using filters and there is more than one column in the report,
  # it may produce this kind of plots
  if(field_name_dd[1]%in% names(data_report_matrix) || filter_condition == "" && report_cols_len > 1) {
  #} else { }
  
  # Loops over all columns in report to create bars for numeric values
  # (one categorical variable in x-axis and a numeric one in y-axis)
    for(k in 1:(report_cols_len - 1)) {
      #  print(c("*** col k in report ***", k))
      first_field_report <- 0
      first_field_dd <- 0
      second_field_report <- 0
      second_field_dd <- 0
      # Looking in data dictionary for first considered column in report
          col_type <- ""
      for(j in 1:dd_len) {
       # print(c("j", j))
       # Checks if field name in column and dd are the same
       if(col_names_report[k] == field_name_dd[j]) {
          # Checks if fields are the plottable type
          if(field_type_dd[j] == 'radio' || field_type_dd[j] == 'dropdown'
            || field_type_dd[j] == 'yesno' || field_type_dd[j] == 'truefalse'
            ) {
            col_type <- "categorical"
            first_field_report <- k
            first_field_dd <- j
          }
          else if(field_type_dd[j] == 'calc'
            || (field_type_dd[j] == 'text' && (
          ...
          if (col_type != "") {
            ...
            # print(c("j", j))
            # print(c("i", i))
            # print(c("col_names_report[k]", col_names_report[k]))
            # print(c("field_name_dd[i]", field_name_dd[i]))
            # print(c("field_name_dd[j]", field_name_dd[j]))
  
            # XYZ: Why is this break action??? Commented by now
            # break
          }
        } # if(col_names_report[i] == field_name_dd[j])
        # print( c("After Checks if fields are the plottable"))
      } # # Loops over all fields in dd to see if they are the correct type and compare field names # for(j in 1:dd_len)
      # If field for loop k is the type needed to graph
      if (col_type != "") {
        rows_second_element <- NULL
        # Loops over all columns in report, starting from next column to review if it's going to be plotted
        for(i in (k+1):report_cols_len) { # if(k < report_cols_len - 1) {
          if(col_type == "numeric") {
            first_field_report <- 0
            first_field_dd <- 0
          }
          ...
          second_col_type <- ""
          # Looking in data dictionary for second considered column in report
          for(j in 1:dd_len) {
          # print(c("j", j))
          # Checks if field name in column and dd are the same
          if(col_names_report[i] == field_name_dd[j]) {
            second_col_type <- ""
            # Checks if fields are the plottable type
            if(col_type == "numeric" && (field_type_dd[j] == 'radio' || field_type_dd[j] == 'dropdown'
              || field_type_dd[j] == 'yesno' || field_type_dd[j] == 'truefalse'
              )   ) {
              second_col_type <- "categorical"
              first_field_report <- i
              first_field_dd <- j
            }
            else if(col_type == "categorical" && (field_type_dd[j] == 'calc'
              || (field_type_dd[j] == 'text' && (
                    text_val_dd[j] == 'integer' ||
                    substr(text_val_dd[j],1,6) == 'number') )
             )   ) {
              second_col_type <- "numeric"
              second_field_report <- i
              second_field_dd <- j
            }
          } # if(col_names_report[i] == field_name_dd[j])
          ...
          #################################################
          ### Graph when both components have been selected and second field is numeric
          #################################################
          if(first_field_report > 0 && second_field_report > 0 &&  second_col_type == "numeric") {
            ### XYZ TO USE IN OTHER TYPES OF GRAPHS: REMOVING NAs
            # Identify rows where any of the two columns is NA using function is.na()
            na_rows <- is.na(data_report_matrix[,first_field_report] * data_report_matrix[,second_field_report])
            # Restore data frame without rows that have NAs (note ! exclamation)
            data_report_wo_blank <- data_report_matrix[!na_rows, c(first_field_report,second_field_report)] 
            #  print( c("data_report_wo_blank", data_report_wo_blank))
  
            y_label <- break_by_words(remove_html(field_label_dd[second_field_dd]), x_title_len)
            x_label <- break_by_words(remove_html(field_label_dd[first_field_dd]), y_title_len)
            count_table <- count_table + 1
    
            first_factor =  factor(data_report_wo_blank[,1])
            second_factor = data_report_wo_blank[,2]
            max_label_size <- 50
    
            # Initiates vector to insert labels of bars included in the graph (only those with non-zero values)
            first_label_vector <- rep(NA, length(unique(data_report_wo_blank[,1]) ) )
            counter_label <- 1
            # print(C("after assigning graph components"))
            # Loop over number of options in first_field_dd
            for(loop in 1:options_number[first_field_dd]) {
              # Loop over options with non-zero values
              for(compare in unique(data_report_wo_blank[,1])) {
                # When option in dd is in the non-zero values insert the corresponding label in the vector of labels
                if(options_value[first_field_dd, loop] == compare) {
                  first_label_vector[counter_label] = options_label[first_field_dd, loop]
                  # str_replace_all(first_label_vector[counter_label], "/", "-")
                  # print( c("loop", loop))
                  # print( c("compare", compare))
                  # print( c("options_value[first_field_dd, loop]", options_value[first_field_dd, loop]))
                  # ...
                  # if label is too large, it trims it
                  if(  nchar( first_label_vector[counter_label])  >   max_label_size  ) {
                    first_label_vector[counter_label] <-
                      paste(  substr(  first_label_vector[counter_label], 1, max_label_size), '...')
                  }
           ...
           # Gets the number of unique values in first_factor, to use for the pallete color
           points_size <- length( unique(data_report_wo_blank[,1]) )
           ...
           graph_title_1 <- paste(field_label_dd[second_field_dd], " (Totals) vs.",
                                 field_label_dd[first_field_dd], sep = " ")
           graph_title_1 <- remove_html(graph_title_1)
    
           if(length(unique(second_factor)) > max_label_size )
           ...
           totals <- NULL
           # print( c("totals after NULL", totals))
           totals <- data.frame(  aggregate(as.numeric(data_w_legend$second_factor), by = list(data_w_legend$first_factor), FUN = sum)  )
           colnames(totals) <- c("group", "value")
    
           # Adjust bars label if too many
           if(nrow(totals) >= 10) {
             x_size = 10
             x_size_bars = 3
           }
           else if(nrow(totals) > 34 ) {
             x_size = 7
             x_size_bars = 2
             legend_size = 5
           }
           ...
           rows_second_element[i] <- nrow(totals)
           # print( c("rows_second_element[i]", rows_second_element[i]))
    
           #######
           #######
           # print( c("x_size ", x_size))
    
           # Calls function to graph with ggplot. IMPORTANT: It enables to pass dynamic parameters
           result_graph <- do.call('ggplot_mine', 
                list(totals[1:nrow(totals),], 
                first_label_vector, 
                totals[1:nrow(totals),2], 
                first_label_vector, 
                full_line, x_size_bars, x_size, y_size, x_label, y_label, x_face, y_face, palette_bar_amount))
           
           # Actually prints the resulting graph
           print(result_graph)
        } # End loop for graph when two fields have been selected:
        # if(first_field_report > 0 && second_field_report > 0)
      } # for(i in (k+1):report_cols_len) 
    } # # Checks if fields are the plottable type # if(field_type_dd[j] == 'radio' || ...
  } # Loops over all columns in report
  } # If report has record id or it's not using filters
...
```{r cross tabs , echo=FALSE, out.width = "100%", fig.align="center"}
          ...
          if(( i > 0 )) {
           # Checks if field name in column and dd are the same
            if(col_names_report[i] == field_name_dd[j]) {
              ...
              # Rename all factor levels for rows in cross tabulation
              if(field_type_dd[first_field_dd] == "yesno" ) {
                row_labels[1] = "No"
                row_labels[2] = "Yes"
              }
              if(field_type_dd[first_field_dd] == "truefalse" ) {
                row_labels[1] = "False"
                row_labels[2] = "True"
              }
  
              # Rename all factor levels for rows & cols in cross tabulation
              if(field_type_dd[second_field_dd] == "yesno" ) {
                col_labels[1] = "No"
                col_labels[2] = "Yes"
              }
              if(field_type_dd[second_field_dd] == "truefalse" ) {
                col_labels[1] = "False"
                col_labels[2] = "True"
              }
              ...
              colnames(cross) <- col_labels
               ...
               # Workaround to be able to print the table outside the loop
                ...
                # Rename all factor levels for rows in cross tabulation if yesno or truefalse fields
                if(field_type_dd[second_field_dd] == "yesno" ) {
                  levels(first_factor) <- c("No", "Yes")
                }
                if(field_type_dd[first_field_dd] == "yesno" ) {
                  levels(second_factor) <- c("No", "Yes")
                }
                if(field_type_dd[second_field_dd] == "truefalse" ) {
                  levels(first_factor) <- c("False","True")
                }
                if(field_type_dd[first_field_dd] == "truefalse" ) {
                  levels(second_factor) <- c("False","True")
                }
...
```{r printing & maps , echo=FALSE, out.width = "100%", fig.align="center", fig.height=8, fig.width=10}
...
if(no_results == '') {
  ...
  if(report_len == 0) {
    no_results_map <- "No results for maps were returned"
  }
...
```{r network graphs , echo=FALSE, out.width = "100%", fig.align="center"}
             ...
             # Checks if field name in column and dd are the same
              if(col_names_report[i] == field_name_dd[j]) {
                ...
                if(first_field_report > 0 && second_field_report > 0) {
                  ...
                  second_factor = factor(data_report_matrix[,first_field_report])
                  ###########
                  # If both variables are not empty, it proceeds to plot the network graph
                  if(is_empty(first_factor, all.na.empty = TRUE) == FALSE && 
                     is_empty(second_factor, all.na.empty = TRUE) == FALSE ) {
                    cross.table.df <- data.frame(first_factor, second_factor)
                    ## Network graph
